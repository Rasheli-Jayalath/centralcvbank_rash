...
@require_once("requires/session.php");
	...
	$eDiscipline			= $_REQUEST['eDiscipline'];
	$eInstitute				= $_REQUEST['eInstitute'];
	$lname					= $_REQUEST['lname'];
	$employed_by			= $_REQUEST['employed_by'];
	$employeer				= $_REQUEST['employeer'];
	$projTitle				= $_REQUEST['projTitle'];
	$dutiesPerformed		= $_REQUEST['dutiesPerformed'];
	$fgroup					= $_REQUEST['fgroup'];
	$email					= $_REQUEST['email'];
	//$txtCountry			= $_REQUEST['nationality'];
...
if($eDiscipline!="")
{
	if($sCondition!="")
	{
	$sCondition.=" AND eDiscipline LIKE '%".$eDiscipline."%'";
	}
	else
	{
	$sCondition=" eDiscipline LIKE '%".$eDiscipline."%'";
	}
}
...
if($lname!="")
{
	if($sCondition!="")
	{
	$sCondition.=" AND lname LIKE '%".$lname."%'";
	}
	else
	{
	$sCondition=" lname LIKE '%".$lname."%'";
	}
}
...
if($eInstitute!="")
{
	if($sCondition!="")
	{
	$sCondition.=" AND eInstitute LIKE '%".$eInstitute."%'";
	}
	else
	{
	$sCondition=" eInstitute LIKE '%".$eInstitute."%'";
	}
}
...
if($employed_by!="")
{
	if($sCondition!="")
	{
	$sCondition.=" AND employed_by LIKE '%".$employed_by."%'";
	}
	else
	{
	$sCondition=" employed_by LIKE '%".$employed_by."%'";
	}
}
...
if($employeer!="")
{
	if($sCondition!="")
	{
	$sCondition.=" AND employeer LIKE '%".$employeer."%'";
	}
	else
	{
	$sCondition=" employeer LIKE '%".$employeer."%'";
	}
}
...
if($projTitle!="")
{
	if($sCondition!="")
	{
	$sCondition.=" AND projTitle LIKE '%".$projTitle."%'";
	}
	else
	{
	$sCondition=" projTitle LIKE '%".$projTitle."%'";
	}
}
...
if($dutiesPerformed!="")
{
	if($sCondition!="")
	{
	$sCondition.=" AND dutiesPerformed LIKE '%".$dutiesPerformed."%'";
	}
	else
	{
	$sCondition=" dutiesPerformed LIKE '%".$dutiesPerformed."%'";
	}
}
...
if($email!="")
{
	if($sCondition!="")
	{
	$sCondition.=" AND email LIKE '%".$email."%'";
	}
	else
	{
	$sCondition=" email LIKE '%".$email."%'";
	}
}
...
//SELECT * FROM querysearch WHERE  eInstitute LIKE '%University%' group by cvid order by cvId desc
...
$sSQL1 = " select * FROM tblcvmain  where startexpyr > 1981 order by cvId desc ";
}

else if($view=='latest')
{
//$sSQL1 = " select cvId, name,position,citizenship FROM tblcvmain order by cvId desc limit 0,50   ";
   $sSQL1 = " select * FROM tblcvmain where startexpyr > 1981 order by cvId desc limit 0,19 ";
}
...
//$sSQL1 = " select cvId, name,position,citizenship FROM tblcvmain order by cvId desc limit 0,50   ";
$sSQL1 = " SELECT * FROM tblcvmain startexpyr > 1981 order by updated_on desc limit 0,25  ";
...
{
 $sSQL1  = "SELECT * FROM querysearch WHERE $sCondition   group by cvid order by cvId desc";
}
...
<div class="card container-fluid" style="margin-top: 15px; overflow-x: auto">
     ...
     <tr style="text-align: center;font-size: 14px;font-weight: bolder;background-color: #c0c0c0; border-bottom:1px solid #d9d9d9;">
			...
			<th style="width: 13%;" class="tablerw">Discipline</th>
...
echo '
<td width="3%" class="tablerw"> Salary<br>Level </td> 
<td width="3%" class="tablerw"> F.Group </td> 
...
<td width="3%" class="tablerw" valign="middle"> Verify <br> Status </td> 
...
if($iCount>0)
{
	...
	$employed_by	=	$objDb->getField($i, employed_by);
	$fgroup			=	$objDb->getField($i, fgroup);
...
$sSQL_p1 = " Select   folder, new_filename FROM tbldocs WHERE cvId='$cvId' AND folder like 'pic%'";
...
<td width="6%" height="23" align="center" style="border-bottom:1px solid #cccccc" ><font color="#990000"> <strong><?=$cvId?> </strong></font></td>
 	   ...
 	   <td width="15%"  style="border-bottom:1px solid #cccccc" align="center"  >
	   <div class="row">
            <div class="col-12">
			  <form action=""  >
...
<td width="5%" class="mg-image" style="border-bottom:1px solid #cccccc" align="center" ><?=$eDiscipline?></td>
...
<?php if ($cvflag==0 and $cvadmflag==1 or $superadminflag==1 or $cventryflag==1) { ?>
     ...
     <?php
		...
		$sSQL_p1 = " Select   folder, new_filename FROM tbldocs WHERE cvId='$cvId' AND folder like 'original%'";
		$objDb3->query($sSQL_p1);
		//$pCount1 = $objDb3->getCount();
