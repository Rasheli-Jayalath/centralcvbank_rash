...
if($updateBtn!='')
{
	...
	$old_filename=mysql_real_escape_string(trim($orignal_name_file));
	  $combine="";
	   $r_title=explode(".",$old_filename);
	  $length=count($r_title);
   ...
   $pCount = $objDb->getCount();
	...
	{
		///upload in category folder
	$DBpicture1 = $objDb->getField(0,new_filename);	
	///end	
...
$resultd=mysql_fetch_array($query_selectd);
	...
	$category_cd=$resultd['category_cd'];
	$cid=$resultd['cid'];
	
$sql_selectd1="select report_id from rs_tbl_documents where cvid=$cvID and report_category=$category_cd and report_file='$DBpicture1'";
$query_selectd1=mysql_query($sql_selectd1); 
$resultd1=mysql_fetch_array($query_selectd1);

	$report_id=$resultd1['report_id'];	
	/////end
...
$tpSql_c = "update rs_tbl_documents SET report_title='$report_title',  report_file='$picture', extension='$ext',doc_creater_id=$user_cd, doc_last_modified_by='$doc_creat_by' where report_id = $report_id";
	
$res_s=mysql_query($tpSql_c);
...
$resultd=mysql_fetch_array($query_selectd);
	...
	$sSQL_c = ("INSERT INTO  rs_tbl_documents (report_category,report_title,report_file,extension,remarks,uploading_file_date,doc_creater,doc_creater_id, cvid) VALUES ($category_cd,'$report_title', '$picture', '$ext','','$posted_date','$doc_creat_by',$user_cd,$cid)");
...
/////////////////////////Signature//////////////////////////
   ...
   $pCount = $objDb->getCount();
	...
	{
	///upload in category folder
	$DBsignature1 = $objDb->getField(0,new_filename);	
	///end		
	$DBsignature = $objDb->getField(0,new_filename);
		...
		///upload in category folder
		$sql_selectd="select category_cd,cid  from rs_tbl_category where cid=$cvID and category_name='Signatures'";
$query_selectd=mysql_query($sql_selectd); 
$resultd=mysql_fetch_array($query_selectd);
...
$sql_selectd1="select report_id from rs_tbl_documents where cvid=$cvID and report_category=$category_cd and report_file='$DBsignature1'";
$query_selectd1=mysql_query($sql_selectd1); 
$resultd1=mysql_fetch_array($query_selectd1);
...
$tpSql_c = "update rs_tbl_documents SET report_title='$report_title',  report_file='$signature', extension='$ext',doc_creater_id=$user_cd, doc_last_modified_by='$doc_creat_by' where report_id = $report_id";
...
$resultd=mysql_fetch_array($query_selectd);
	...
	$sSQL_c = ("INSERT INTO  rs_tbl_documents (report_category,report_title,report_file,extension,remarks,uploading_file_date,doc_creater,doc_creater_id, cvid) VALUES ($category_cd,'$report_title', '$signature', '$ext','','$posted_date','$doc_creat_by',$user_cd,$cid)");
$res_s=mysql_query($sSQL_c);
...
/////////////////////////Orignal CV//////////////////////////
   ...
   $pCount = $objDb->getCount();
	...
	{
	///upload in category folder
	$DBoriginalcv1 = $objDb->getField(0,new_filename);	
	///end		
	$DBoriginalcv = $objDb->getField(0,new_filename);
		...
		///upload in category folder
		$sql_selectd="select category_cd,cid  from rs_tbl_category where cid=$cvID and category_name='CVs'";
$query_selectd=mysql_query($sql_selectd); 
$resultd=mysql_fetch_array($query_selectd);
...
$sql_selectd1="select report_id from rs_tbl_documents where cvid=$cvID and report_category=$category_cd and report_file='$DBoriginalcv1'";
$query_selectd1=mysql_query($sql_selectd1); 
$resultd1=mysql_fetch_array($query_selectd1);
...
$tpSql_c = "update rs_tbl_documents SET report_title='$report_title',  report_file='$originalcv', extension='$ext',doc_creater_id=$user_cd, doc_last_modified_by='$doc_creat_by' where report_id = $report_id";
...
$resultd=mysql_fetch_array($query_selectd);
	...
	$sSQL_c = ("INSERT INTO  rs_tbl_documents (report_category,report_title,report_file,extension,remarks,uploading_file_date,doc_creater,doc_creater_id, cvid) VALUES ($category_cd,'$report_title', '$originalcv', '$ext','Original CV','$posted_date','$doc_creat_by',$user_cd,$cid)");
$res_s=mysql_query($sSQL_c);
...
if($cvID!="")
{
	 $sSQL_p = " Select * FROM tbldocs WHERE cvId='$cvID' AND folder like 'pic%'";
	 $objDb->query($sSQL_p);
	 $pCount = $objDb->getCount();
	 if($pCount>0)
	 {
	 ...
	 $sSQL_s = " Select * FROM tbldocs WHERE cvId='$cvID' AND folder like 'signature%'";
	$objDb->query($sSQL_s);
	 ...
	 $sSQL_c = " Select * FROM tbldocs WHERE cvId='$cvID' AND folder like 'original%'";
	$objDb->query($sSQL_c);
...
<div class="main-panel">
  <div class="content-wrapper">
	...
	<form name="uploadcv" id="uploadcv" action=""  method="post" onsubmit="return uploadcv(this);" style="  margin-top:20px"  enctype="multipart/form-data">
	  
	  <table width="96%" height="660" border="0"  align="center" cellpadding="1" cellspacing="1">
...
<?	  	    $sSQL_name = " Select name FROM tblcvmain WHERE cvId='$cvID' ";
	...
	?>
        ...
        <!-- </font>-->  </td>
					...
					<tr>
					<td height="31" colspan="3"><h3> <strong>Upload  Documents/ Particulars:</strong></h3></td>
					<td colspan="42"><font color="#009933"><strong><?php if($msg!="") echo $msg; else echo "";?></strong></font></td>
					</tr>

					<tr>
					  <td width="22%" height="68" class="label" > 
                     <h3> <strong>Picture:</strong></h3>  
                      ...
                      <font color="#FF0000"; size="-1">Only <strong>jpg format</strong> file can be uploaded.
					  <br> 
					  <b>Photo</b> should be less then <b>2MB.</b> </font> </td>
					  <td width="18%" >
      <div class="mg-image"> 
  		<?php 
		$piclen = strlen($pic);
		...
		?>
					...
					<tr>
					  <td height="83"   class="label" ><h3><strong>Signature</strong>:</h3></td>
					  <td colspan="2" ><input type="file" name="signature" accept="image/jpg, image/jpeg"  />
                      <font color="#CCCCCC"><? echo $odbsignature;?></font> <br />
                      <font color="#FF0000"; size="-1">Only <strong>jpg Format</strong> file can be uploaded.</font>
		...
		?>
           ...
           <tr>
		   <td height="85" class="label"><h3><strong>Original CV</strong>:</h3></td>
           <td colspan="2" class="smec">
             ...
             <font color="#FF0000"; size="-1"> Only <strong> doc/pdf </strong> file  can be uploaded.</font><a href="<? echo $ocv; ?>" title="Original CV" > </a>           </td>
           <td class="smec">
          ...
          <?php 
           ...
           <?php /*?><tr>
		   <td height="85" class="label"><h3><strong>NID/Passport:</strong></h3></td>
           <td colspan="2" class="smec">
             ...
             <font color="#FF0000"; size="-1">Only <strong>pdf format</strong> file can be uploaded.</font>
         ...
         <?php /*?><tr>
		   <td height="85" class="label"><h3><strong>Educational  Docs:</strong></h3></td>
           <td colspan="2" class="smec">
  <input type="file" name="edudocs" accept="application/msword, application/vnd.ms-excel, application/vnd.ms-powerpoint, text/plain, application/pdf, image/jpg, image/jpeg" size="50" maxlength="50" />
          ...
          <?php 
            ...
            <td> </td>
                 ...
                 <?php /*?><tr>
		   <td height="85" class="label"><h3><strong>Experience Docs:</strong></h3></td>
           <td colspan="2" class="smec">
